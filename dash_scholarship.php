...
if (isset($_GET['del_id']) && check_CSRF()) {
	if (!empty($_GET["del_id"])) {
        $query = "DELETE FROM Scholarship_reg WHERE scholarship_id=". mysqli_real_escape_string($dbc, $_GET["del_id"])
        ...
        ."\" COLLATE utf8_bin AND table_name='Scholarships' COLLATE utf8_bin";
        $data = mysqli_query($dbc,$query);
        //echo $query;
        echo mysqli_error($dbc);
      ...
      <div class="row row-offcanvas row-offcanvas-right">
        ...
        <div class="col-xs-12 col-sm-9">
          ...
          <?php
              ...
              $query = "SELECT * FROM Scholarships WHERE scholarship_id IN (SELECT scholarship_id FROM Scholarship_reg WHERE stuID='".$_SESSION['user_id']."' COLLATE utf8_bin)";
                  ...
                  while($scholarship_info = $result->fetch_array())
                  {
                    ...
                    <div class="row">
                      ...
                      <div class="col-xs-12 col-sm-2 col-md-3">
                      	<hr>
                      	<a href="<?php echo $_SERVER['PHP_SELF'] ?>?scholarship_id=<?php echo $scholarship_info['scholarship_id']?>"><button style="margin-top: 5px" class="btn btn-primary">details</button></a>
                      	<a href="<?php echo $_SERVER['PHP_SELF'] ?>?del_id=<?php echo $scholarship_info['scholarship_id']?>&CSRFcheck=<?php echo $_SESSION['CSRFcheck'] ?>"><button style="margin-top: 5px" class="btn btn-default">withdraw</button></a>
                      </div>
                    </div>
                    <?php
                  }
                  if ($tmp == 0) {
                  	echo "<h2>No scholarship found.</h2>";
                  }
                  gen_pagination();
              }
            ...
            <div class="row">
              <div class="col-xs-12 col-sm-12">
                <?php gen_scholarship_details($dbc, $_GET['scholarship_id'], "Scholarships", "scholarship_id"); ?>
                <hr>
              </div>
              
            </div>
            <?php gen_reg_form($dbc, $_GET['scholarship_id']); ?>
            <?php
          }
          ?>
          </div>
