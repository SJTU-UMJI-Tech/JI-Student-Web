...
<script type="text/javascript">
	$(document).ready(function ()
	{
		...
		for (var i = 1; i <= 9; i++)
		{
			var url = "http://vip.zjqq.mobi/index.php/vote/voteList.html?id=2346&page=" + i + "&num=5&_=" + (new Date()).getTime();
			$.ajax({
				type: "get",
				url: url,
				dataType: "jsonp",
				success: function (data)
				{
					...
					for (var index in data.list)
					{
						...
						list.push({name: obj.user.name, score: obj.vote.score});
						if(obj.voteid=='45229')
						{
					...
					if (++num >= 9)
					{
						...
						for (var j in list)
						{
							var html = '<div class="row">' +
							           '<div class="col-xs-2">' + (Number(j) + 1) + '</div>' +
							           '<div class="col-xs-7">' + list[j].name + '</div>' +
							           '<div class="col-xs-3">' + list[j].score + '</div>' +
							           '</div>';
							$("#list").append(html);
						}
