...
function send_one_email($from, $to_whom, $subject, $context)
{
	$mail = new MySendMail();
	$mail->setServer("smtp.exmail.qq.com", $from."@noreply.umji.online", "Student1st", 465, true); //设置smtp服务器，到服务器的SSL连接
	$mail->setFrom($from."@noreply.umji.online"); //设置发件人
	$mail->setReceiver($to_whom); //设置收件人，多个收件人，调用多次
	$mail->setMail($subject, $context); //设置邮件主题、内容
	$mail->sendMail(); //发送
}
